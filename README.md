...
4. Copy `dcdl/example_settings.py` into a new file called `dcdl/settings.py`. Then configure the  copy (i.e. `settings.py`, which is not tracked by git). Currently, the example settings assume a Postgres backend, so you need to enter the Django secret key and the password for django_user and confirm that the database name is correct (i.e. dcdl_dev for a dev version, dcdl_test for a test version, and dcdl_prod for production).
...
4. Upload reels of images to the database using the methods in `EntryApp/load_db.py`. See **Loading and assigning reels** for instructions. 
...
All backend DBs are owned by a generic user called `django_user`. All passwords are stored in a password-protected Excel file in the CARRA drive.
...
- orders the reels in descending order by keyer count, so that reels with one keyer assigned already are preferred to reels with no keyers assigned
...
Next, `assign_reel()` will create an Image instance for each ImageFile in the reel for the keyer.  
